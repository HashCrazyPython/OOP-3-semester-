...
// Linear search O(N) (If there's method to find int in set of pair, then that function must be changed!)
...
public:
    ...
    // recursive parse of regular expression with creating binary parse tree
    ...
    pairs firstpos(vertex* step, pairs& res) {
        if(step != nullptr) {
    ...
    pairs lastpos(vertex* step, pairs& res) {
        if(step != nullptr) {
            if(isalpha(step->letter) || step->letter == '#') {
                res.insert(pair<int, char>(step->number, step->letter));
            }
    ...
    pairs followpos(int i, vertex* step, pairs& res) {
        pairs temp1;
        pairs temp2;
        if(step->left != nullptr)
                ...
                if(l_search(step, let)) {
                    ...
                    if(!positions.empty()) {
                        ...
                        for(auto tmp: positions) {
                            for(auto pairs : tree.followpos(tmp, tree.get_root(), buff))
